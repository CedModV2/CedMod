...
namespace CedMod.Patches
{
    ...
    public static class RaPlayerPatch
    {
        ...
        public static IEnumerator<float> RaPlayerCoRoutine(RaPlayer __instance, CommandSender sender, string data)
        {
            ...
            if (source[1].StartsWith("-1") && CommandProcessor.CheckPermissions(sender, PlayerPermissions.BanningUpToDay))
            {
                Timing.RunCoroutine(HandleReportType1(sender, player, source));
                yield break;
            }
            
            if (source[1].StartsWith("-2") && CommandProcessor.CheckPermissions(sender, PlayerPermissions.BanningUpToDay))
            {
