...
namespace CedMod
{
    public class FriendlyFireAutoBan
    {
        ...
        public FriendlyFireAutoBan(Plugin plugin) => Plugin = plugin;
        ...
        public void ConsoleCommand(ConsoleCommandEvent ev)
        {
            ...
            {
                case "FORGIVE":
                    if (_victims.ContainsKey(ev.Player.characterClassManager.UserId))
                    {
                        string killeruid = _victims.GetValueSafe(ev.Player.characterClassManager.UserId);
                        ...
                        if (hub1 != null)
                        {
                            ev.ReturnMessage = "You have forgiven " + hub1.GetNickname() + "(" +
                                               hub1.characterClassManager.UserId + ").";
                            Timing.KillCoroutines("timer");
                            _victims.Remove(ev.Player.characterClassManager.UserId);
                            hub1.Broadcast(10,
                                "<color=yellow>You have been forgiven by " + ev.Player.GetNickname() + "(" +
                                ev.Player.characterClassManager.UserId + ")" + ".</color>");
