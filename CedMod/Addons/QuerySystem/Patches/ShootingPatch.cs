        ...
        public static bool Prefix(ShotBacktrackData __instance, Firearm firearm, Action<ReferenceHub> processingMethod)
        {
            ...
            try
            {
                ...
                using (new FpcBacktracker(firearm.Owner, claimedPos, claimedRot))
                {
                    ...
                    {
                        var ping = __instance.PrimaryTargetHub.authManager.InstanceMode == ClientInstanceMode.ReadyClient ? (LiteNetLib4MirrorServer.Peers[firearm.Owner.connectionToClient.connectionId].Ping * 2 / (float)1000) + 0.05f : 0.4f;
